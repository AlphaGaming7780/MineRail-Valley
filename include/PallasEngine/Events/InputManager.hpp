...
namespace pallas {
    ...
    private:
        InputManager() 
        {
            ...
            m_MouseMoved =          ev.on<sf::Event::MouseMoved>            ([this](const sf::Event::MouseMoved ev)             {this->onMouseMoved(ev); });
            m_keyPressed =          ev.on<sf::Event::KeyPressed>            ([this](const sf::Event::KeyPressed ev)             {this->onKeyPressed(ev.scancode); });
            m_keyReleased =         ev.on<sf::Event::KeyReleased>           ([this](const sf::Event::KeyReleased ev)            {this->onKeyReleased(ev.scancode);});
            m_MouseButtonPressed =  ev.on<sf::Event::MouseButtonPressed>    ([this](const sf::Event::MouseButtonPressed ev)     {this->onMousePressed(ev); });
            m_MouseButtonReleased = ev.on<sf::Event::MouseButtonReleased>   ([this](const sf::Event::MouseButtonReleased ev)    {this->onMouseReleased(ev);});
			m_MouseWheelMoved =     ev.on<sf::Event::MouseWheelScrolled>    ([this](const sf::Event::MouseWheelScrolled ev)     {this->onMouseWheelMoved(ev); });
        }
        ...
        void onKeyPressed(sf::Keyboard::Scan key);
        void onKeyReleased(sf::Keyboard::Scan key);
    ...
    private:
        ...
        Logger m_Logger = Logger("InputManager");
        ...
        EventManager::ListenerId m_MouseMoved = 0;
        EventManager::ListenerId m_keyPressed = 0;
        EventManager::ListenerId m_keyReleased = 0;
        EventManager::ListenerId m_MouseButtonPressed = 0;
        EventManager::ListenerId m_MouseButtonReleased = 0;
		EventManager::ListenerId m_MouseWheelMoved = 0;
