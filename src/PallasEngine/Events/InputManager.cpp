...
namespace pallas
{
	InputManager::~InputManager()
    {
        ...
        ev.off(m_MouseMoved);
        ev.off(m_keyPressed);
        ev.off(m_keyReleased);
        ev.off(m_MouseButtonPressed);
        ev.off(m_MouseButtonReleased);
    }
    ...
    void InputManager::onKeyPressed(sf::Keyboard::Scan key)
    {
    ...
    void InputManager::onKeyReleased(sf::Keyboard::Scan key)
    {
        if (!m_EnableKeyboardInputs) return;
        for (auto& [type, block] : blocks) {
            if (!block.enabled) continue;
